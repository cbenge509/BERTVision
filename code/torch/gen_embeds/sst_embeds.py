...
from data.bert_processors.processors import SST, Tokenize_Transform
...
def emit_train_embeddings(dataloader, train_dataset, model, device, args):
    ...
    with h5py.File(save_location + 'sst_bert_embeds.h5', 'w') as f:
    ...
    with h5py.File(save_location + 'sst_labels.h5', 'w') as l:
    ...
    with h5py.File(save_location + 'sst_idx.h5', 'w') as i:
        # create empty data set; [batch_sz]
    ...
    for step, batch in enumerate(dataloader):
        ...
        # add embeds to ds
        with h5py.File(save_location + 'sst_bert_embeds.h5', 'a') as f:
            # initialize dset
            dset = f['embeds']
        ...
        # add labels to ds
        with h5py.File(save_location + 'sst_labels.h5', 'a') as l:
            # initialize dset
            label_dset = l['labels']
            # counter to add chunk of rows
            start = step*args.embed_batch_size
        ...
        # add idx to ds
        with h5py.File(save_location + 'sst_idx.h5', 'a') as i:
            # initialize dset
            idx_dset = i['idx']
            # counter to add chunk of rows
            start = step*args.embed_batch_size
...
def emit_dev_embeddings(dataloader, train_dataset, model, device, args):
    ...
    with h5py.File(save_location + 'sst_dev_bert_embeds.h5', 'w') as f:
        # create empty data set; [batch_sz, layers, tokens, features]
        dset = f.create_dataset('embeds', shape=(num_documents, args.n_layers, args.max_seq_length, args.n_features),
                                maxshape=(None, args.n_layers, args.max_seq_length, args.n_features),
                                chunks=(args.embed_batch_size, args.n_layers, args.max_seq_length, args.n_features),
    ...
    with h5py.File(save_location + 'sst_dev_labels.h5', 'w') as l:
        # create empty data set; [batch_sz]
        label_dset = l.create_dataset('labels', shape=(num_documents,),
                                      maxshape=(None,), chunks=(args.embed_batch_size,),
                                      dtype=np.int64)

    with h5py.File(save_location + 'sst_dev_idx.h5', 'w') as i:
        # create empty data set; [batch_sz]
        idx_dset = i.create_dataset('idx', shape=(num_documents,),
                                      maxshape=(None,), chunks=(args.embed_batch_size,),
                                      dtype=np.int64)
    ...
    for step, batch in enumerate(dataloader):
        ...
        # add embeds to ds
        with h5py.File(save_location + 'sst_dev_bert_embeds.h5', 'a') as f:
            # initialize dset
            dset = f['embeds']
            # counter to add chunk of rows
            start = step*args.embed_batch_size
        ...
        # add labels to ds
        with h5py.File(save_location + 'sst_dev_labels.h5', 'a') as l:
            # initialize dset
            label_dset = l['labels']
            # counter to add chunk of rows
            start = step*args.embed_batch_size
        ...
        # add idx to ds
        with h5py.File(save_location + 'sst_dev_idx.h5', 'a') as i:
            # initialize dset
            idx_dset = i['idx']
            # counter to add chunk of rows
            start = step*args.embed_batch_size
    ...
    # check data
    with h5py.File(save_location + 'sst_dev_bert_embeds.h5', 'r') as f:
        print('last embed batch entry', f['embeds'].attrs['last_index'])
        print('embed shape', f['embeds'].shape)
        print('last entry:', f['embeds'][-1, :, :, :])

    with h5py.File(save_location + 'sst_dev_labels.h5', 'r') as l:
        print('last embed batch entry', l['labels'].attrs['last_index'])
        print('embed shape', l['labels'].shape)
        print('last entry:', l['labels'][len(train_dataset)-10: len(train_dataset)])
...
def main():
    ...
    def get_args():
        ...
        parser.add_argument('--max-seq-length', type=int, default=66, metavar='N',
