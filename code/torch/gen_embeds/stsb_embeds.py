    ...
    def __call__(self, sample):
        ...
        if 'text2' not in sample:
            encodings = self.tokenizer(
                                ...
                                max_length=70,  # set max length; SST is 64
...
def emit_embeddings(dataloader, train_dataset, model, device, args):
    ...
    with h5py.File('C:\\w266\\data2\\h5py_embeds\\stsb_bert_embeds.h5', 'w') as f:
    ...
    with h5py.File('C:\\w266\\data2\\h5py_embeds\\stsb_labels.h5', 'w') as l:
    ...
    with h5py.File('C:\\w266\\data2\\h5py_embeds\\stsb_idx.h5', 'w') as i:
        # create empty data set; [batch_sz]
    ...
    for step, batch in enumerate(dataloader):
        ...
        # add embeds to ds
        with h5py.File('C:\\w266\\data2\\h5py_embeds\\stsb_bert_embeds.h5', 'a') as f:
            dset = f['embeds']
            # add chunk of rows
        ...
        # add labels to ds
        with h5py.File('C:\\w266\\data2\\h5py_embeds\\stsb_labels.h5', 'a') as l:
            label_dset = l['labels']
            # add chunk of rows
            start = step*args.embed_batch_size
            # [batch_sz, layer, tokens, features]
        ...
        # add idx to ds
        with h5py.File('C:\\w266\\data2\\h5py_embeds\\stsb_idx.h5', 'a') as i:
            idx_dset = i['idx']
            # add chunk of rows
            start = step*args.embed_batch_size
            # [batch_sz, layer, tokens, features]
    ...
    # check data
    with h5py.File('C:\\w266\\data2\\h5py_embeds\\stsb_bert_embeds.h5', 'r') as f:
        print('last embed batch entry', f['embeds'].attrs['last_index'])
        print('embed shape', f['embeds'].shape)
        print('last entry:', f['embeds'][-1, :, :, :])

    with h5py.File('C:\\w266\\data2\\h5py_embeds\\stsb_labels.h5', 'r') as l:
...
def main():
    ...
    parser.add_argument('--num-labels', type=int, default=1, metavar='N',
                         help='number of labels to classify - STSB is Regression (default: 1)')
    parser.add_argument('--l2', type=float, default=0.01, metavar='LR',
                         help='l2 regularization weight (default: 0.01)')
    parser.add_argument('--max-seq-length', type=int, default=70, metavar='N',
                         help='max sequence length for encoding (default: 70)')
    ...
    for epoch in range(1, epochs + 1):
        ...
        if logs['loss'] < best_loss:
            # torch save
            torch.save(model.state_dict(), 'C:\\w266\\data2\\checkpoints\\BERT-STSB' + '_epoch_{}.pt'.format(epoch))
            best_loss = logs['loss']
    ...
    model.load_state_dict(torch.load('C:\\w266\\data2\\checkpoints\\BERT-STSB_epoch_1.pt'))
