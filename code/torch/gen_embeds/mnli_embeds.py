...
from data.bert_processors.processors import MNLI, Tokenize_Transform
...
def emit_dev_embeddings(dataloader, train_dataset, model, device, args):
    ...
    with h5py.File(save_location + 'mnli_dev_matched_bert_embeds.h5', 'w') as f:
        # create empty data set; [batch_sz, layers, tokens, features]
        dset = f.create_dataset('embeds', shape=(num_documents, args.n_layers, args.max_seq_length, args.n_features),
                                maxshape=(None, args.n_layers, args.max_seq_length, args.n_features),
                                chunks=(args.embed_batch_size, args.n_layers, args.max_seq_length, args.n_features),
    ...
    with h5py.File(save_location + 'mnli_dev_matched_labels.h5', 'w') as l:
        # create empty data set; [batch_sz]
        label_dset = l.create_dataset('labels', shape=(num_documents,),
                                      maxshape=(None,), chunks=(args.embed_batch_size,),
                                      dtype=np.int64)

    with h5py.File(save_location + 'mnli_dev_matched_idx.h5', 'w') as i:
        # create empty data set; [batch_sz]
        idx_dset = i.create_dataset('idx', shape=(num_documents,),
                                      maxshape=(None,), chunks=(args.embed_batch_size,),
                                      dtype=np.int64)
    ...
    for step, batch in enumerate(dataloader):
        ...
        # add embeds to ds
        with h5py.File(save_location + 'mnli_dev_matched_bert_embeds.h5', 'a') as f:
            # initialize dset
            dset = f['embeds']
            # counter to add chunk of rows
            start = step*args.embed_batch_size
        ...
        # add labels to ds
        with h5py.File(save_location + 'mnli_dev_matched_labels.h5', 'a') as l:
            # initialize dset
            label_dset = l['labels']
            # counter to add chunk of rows
            start = step*args.embed_batch_size
        ...
        # add idx to ds
        with h5py.File(save_location + 'mnli_dev_matched_idx.h5', 'a') as i:
            # initialize dset
            idx_dset = i['idx']
            # counter to add chunk of rows
            start = step*args.embed_batch_size
    ...
    # check data
    with h5py.File(save_location + 'mnli_dev_matched_bert_embeds.h5', 'r') as f:
        print('last embed batch entry', f['embeds'].attrs['last_index'])
        print('embed shape', f['embeds'].shape)
        print('last entry:', f['embeds'][-1, :, :, :])

    with h5py.File(save_location + 'mnli_dev_matched_labels.h5', 'r') as l:
        print('last embed batch entry', l['labels'].attrs['last_index'])
        print('embed shape', l['labels'].shape)
        print('last entry:', l['labels'][len(train_dataset)-10: len(train_dataset)])
...
# prepare embedding extraction
def emit_dev_embeddings2(dataloader, train_dataset, model, device, args):
    # timing metrics
    t0 = time.time()
    batch_num = args.embed_batch_size
    num_documents = len(train_dataset)
    ...
    with h5py.File(save_location + 'mnli_dev_mismatched_bert_embeds.h5', 'w') as f:
        # create empty data set; [batch_sz, layers, tokens, features]
        dset = f.create_dataset('embeds', shape=(num_documents, args.n_layers, args.max_seq_length, args.n_features),
                                maxshape=(None, args.n_layers, args.max_seq_length, args.n_features),
                                chunks=(args.embed_batch_size, args.n_layers, args.max_seq_length, args.n_features),
    ...
    with h5py.File(save_location + 'mnli_dev_mismatched_labels.h5', 'w') as l:
        # create empty data set; [batch_sz]
        label_dset = l.create_dataset('labels', shape=(num_documents,),
                                      maxshape=(None,), chunks=(args.embed_batch_size,),
                                      dtype=np.int64)

    with h5py.File(save_location + 'mnli_dev_mismatched_idx.h5', 'w') as i:
        # create empty data set; [batch_sz]
        idx_dset = i.create_dataset('idx', shape=(num_documents,),
                                      maxshape=(None,), chunks=(args.embed_batch_size,),
                                      dtype=np.int64)
    ...
    for step, batch in enumerate(dataloader):
        ...
        # add embeds to ds
        with h5py.File(save_location + 'mnli_dev_mismatched_bert_embeds.h5', 'a') as f:
            # initialize dset
            dset = f['embeds']
            # counter to add chunk of rows
            start = step*args.embed_batch_size
        ...
        # add labels to ds
        with h5py.File(save_location + 'mnli_dev_mismatched_labels.h5', 'a') as l:
            # initialize dset
            label_dset = l['labels']
            # counter to add chunk of rows
            start = step*args.embed_batch_size
        ...
        # add idx to ds
        with h5py.File(save_location + 'mnli_dev_mismatched_idx.h5', 'a') as i:
            # initialize dset
            idx_dset = i['idx']
            # counter to add chunk of rows
            start = step*args.embed_batch_size
    ...
    # check data
    with h5py.File(save_location + 'mnli_dev_mismatched_bert_embeds.h5', 'r') as f:
        print('last embed batch entry', f['embeds'].attrs['last_index'])
        print('embed shape', f['embeds'].shape)
        print('last entry:', f['embeds'][-1, :, :, :])

    with h5py.File(save_location + 'mnli_dev_mismatched_labels.h5', 'r') as l:
        print('last embed batch entry', l['labels'].attrs['last_index'])
        print('embed shape', l['labels'].shape)
        print('last entry:', l['labels'][len(train_dataset)-10: len(train_dataset)])
...
def main():
    ...
    def get_args():
        ...
        parser.add_argument('--max-seq-length', type=int, default=114, metavar='N',
                             help='max sequence length for encoding (default: 114)')
    ...
    # build ds
    train_ds = MNLI(type='train', transform=Tokenize_Transform(args, logger))

    # build ds
    dev_ds_matched = MNLI(type='dev_matched', transform=Tokenize_Transform(args, logger))

    # build ds
    dev_ds_mismatched = MNLI(type='dev_mismatched', transform=Tokenize_Transform(args, logger))
    ...
    # create embed dataloader
    dev_embed_matched_dataloader = DataLoader(dev_ds_matched,
                                batch_size=args.embed_batch_size,
                                shuffle=True,
                                num_workers=args.num_workers,
                                drop_last=False)
    ...
    # create embed dataloader
    dev_embed_mismatched_dataloader = DataLoader(dev_ds_mismatched,
                                batch_size=args.embed_batch_size,
                                shuffle=True,
                                num_workers=args.num_workers,
                                drop_last=False)
    ...
    emit_dev_embeddings(dev_embed_matched_dataloader, dev_ds_matched, model, device, args)
    emit_dev_embeddings2(dev_embed_mismatched_dataloader, dev_ds_mismatched, model, device, args)
