...
def emit_embeddings(dataloader, train_dataset, model, device, args):
    ...
    with h5py.File('C:\\w266\\data2\\h5py_embeds\\cola_dev_bert_embeds.h5', 'w') as f:
    ...
    with h5py.File('C:\\w266\\data2\\h5py_embeds\\cola_dev_labels.h5', 'w') as l:
    ...
    with h5py.File('C:\\w266\\data2\\h5py_embeds\\cola_dev_idx.h5', 'w') as i:
        # create empty data set; [batch_sz]
    ...
    for step, batch in enumerate(dataloader):
        ...
        # add embeds to ds
        with h5py.File('C:\\w266\\data2\\h5py_embeds\\cola_dev_bert_embeds.h5', 'a') as f:
            dset = f['embeds']
            # add chunk of rows
        ...
        # add labels to ds
        with h5py.File('C:\\w266\\data2\\h5py_embeds\\cola_dev_labels.h5', 'a') as l:
            label_dset = l['labels']
            # add chunk of rows
            start = step*args.embed_batch_size
            # [batch_sz, layer, tokens, features]
        ...
        # add idx to ds
        with h5py.File('C:\\w266\\data2\\h5py_embeds\\cola_dev_idx.h5', 'a') as i:
            idx_dset = i['idx']
            # add chunk of rows
            start = step*args.embed_batch_size
            # [batch_sz, layer, tokens, features]
    ...
    # check data
    with h5py.File('C:\\w266\\data2\\h5py_embeds\\cola_dev_bert_embeds.h5', 'r') as f:
        print('last embed batch entry', f['embeds'].attrs['last_index'])
        print('embed shape', f['embeds'].shape)
        print('last entry:', f['embeds'][-1, :, :, :])

    with h5py.File('C:\\w266\\data2\\h5py_embeds\\cola_dev_labels.h5', 'r') as l:
