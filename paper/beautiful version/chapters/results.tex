...
\lettrineabstract{We evaluate the effectiveness of our approach on two common NLP tasks from the SQuAD 2.0 dataset: Q\&A span annotation and binary classification.}
...
\subsection{BERT fine-tuning as baseline}
...
\subsection{Models trained at 3/10 epoch embeddings}
...
\begin{table}[ht]
	\centering
	\small
	\begin{tabular}{L{3.2cm}|C{1.6cm} C{0.9cm} C{0.9cm}}
		\toprule
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=8.5cm]{images/1_Epoch_Embeddings__Adapter_386_with_Skip.png}
	\caption{\label{fig:1_epoch_embeddings__adapter_386_with_skip}Training with less data}
\end{figure}

\subsection{Ensembling our models with BERT}
...
\begin{table}[ht]
	\centering
	\small
	\begin{tabular}{L{4.5cm}|C{0.9cm} C{0.9cm}}
		\toprule
...
\subsection{Models trained at 2/10 epoch embeddings}
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=8.5cm]{images/BinaryClassification_Tenney_Small_2_10ths_epochs_BERT_fine_tuned_Performance_plot.png}
	\caption{\label{fig:bc_2_10ths_performance}2/10ths epoch model vs BERT}
\end{figure}

\subsection{Models trained at 1 epoch embeddings}
...
\begin{table}[ht]
	\centering
	\scriptsize
	\begin{tabular}{C{0.17cm}|C{0.8cm} C{0.8cm}|C{0.8cm} C{0.8cm}|C{1cm} C{1cm}}
		\toprule
		\boldmath$e$ & \textbf{BERT F1} & \textbf{BERT EM} & \textbf{Our F1} & \textbf{Our EM} & \textbf{F1 $\Delta$} & \textbf{EM $\Delta$} \\
		\midrule
		$1$ & \textcolor{calberkeleyblue}{$0.720$} & \textcolor{calberkeleyblue}{$0.761$} & $0.763$ & $0.782$ & \textcolor{callaplane}{\boldmath$+0.043$} & \textcolor{callaplane}{\boldmath$+0.021$} \\
		$3$ & \textcolor{calberkeleyblue}{$0.790$} & \textcolor{calberkeleyblue}{$0.804$} & $0.795$ & $0.811$ & \textcolor{callaplane}{\boldmath$+0.005$} & \textcolor{callaplane}{\boldmath$+0.007$} \\
		\bottomrule
	\end{tabular}
...
\subsection{Modeling with 3 full epoch embeddings}
